...
// Delta Error fuzzification (5 sets)
...
// Rule base (simplified example)
fn fuzzy_inference(mu_err: [f32;7], mu_de: [f32;5], mu_rpm: [f32;3]) -> [f32;5] {
    let mut out = [0.0; 5]; // NB, NS, ZE, PS, PB

    // Example rules:
    // Error PB & RPM Low → PB
    out[4] = f32::max(out[4], mu_err[6].min(mu_rpm[0]));

    // Error ZE & RPM Medium → ZE
    out[2] = f32::max(out[2], mu_err[3].min(mu_rpm[1]));

    // Error NS & Delta Error NB → NB
    out[0] = f32::max(out[0], mu_err[2].min(mu_de[0]));

    // Error PS & Delta Error PB → PS
    out[3] = f32::max(out[3], mu_err[4].min(mu_de[4]));
...
let duty_corr_f = defuzzify(mu_out);
let duty_corr = duty_corr_f as i32;

let mut duty_target = (duty_ff + duty_corr).clamp(0, ARR);
